...
def main():
    ...
    parser.add_argument('-s', '--compress_single', type=bool, default=1, help='compress one parameter at a time')
    ...
    print("\n extracting %d summaries per parameter"%(config["n_summaries"]["cls"]))

    key = jr.PRNGKey(4)
    cls_single_shape = (10, 2, 4, 28,)


    if args.compress_single:

        print('training one parameter at a time')

        ws = []
        all_losses = []

        for j in range(config["n_params"]):

            print('training for parameter %d'%(j))

            n_summ = int(config["n_summaries"]["cls"]) #// config["n_params"]

            model = ClsModel(n_summaries=n_summ,
                            slice_cls_single=slice_cls_single,
                            n_hidden_mdn=config["cls"]["n_hidden_mdn"])

            w = model.init(key, cls_train[0], jnp.ones(1,), method=model.log_prob)


            w, losses = run_training_loop(model, key, (cls_train, params_train[:, j][:, np.newaxis]),
                                                (cls_test, params_test[:, j][:, np.newaxis]), 
                                                    learning_rate=1e-5,
                                                    schedule=True,
                                                    n_params=1,
        ...
        # rewrite in order to save everything consistently
        w = ws
        losses = all_losses
        ...
        print('training all three parameters')
    ...
    print("saving everything to ", outdir)
    ...
    with open('config_cls.yaml', 'w') as outfile:
