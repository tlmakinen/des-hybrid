...
class simplePatchCNN_convonly(nn.Module):
    """CNN to extract extra information from WL field"""
    filters: Sequence[int]
    cls_compression: Callable
    n_extra: int 
    ...
    def __call__(self, x):
        ...
        # RIGHT SO HERE WE HAVE EXISTING CLS NUMBERS AND OLD SUMMARIES
        # TAKE THE PATCH A SUMMARIES AND VARY THE CLS COMPRESSION UNDER NOISE
        existing_info = inputs["A"]["summaries"] 
        ...
        for i in range(6):
            x = nn.Conv(features=filters, kernel_size=(3,3), strides=(1,1), padding="SAME", \
                        dtype=self.dtype)(x) #, kernel_init=nn.initializers.he_normal())(x)
            x = self.act_cnn(x)
            x = nn.avg_pool(x, (2,2), strides=(2,2))
...
class MPK_CNN(nn.Module):
    """CNN to extract extra information from WL field"""
    filters: Sequence[int]
    cls_compression: Callable
    multipole_layer: MPK_layer
    n_extra: int 
    n_summaries_cls: int
    n_existing: int
    n_total: int
...
if __name__ == "__main__":
    ...
    if bool(config["single_param"]):
        ...
        cls_model = ClsModel(n_summaries=config["n_summaries"]["cls"] // config["n_params"],
...
# ----------------------------------------------------------------------------
    ...
    if config["patch_net"]["model_type"] == "conv_only":
        modeltype = "conv_only"
        print("using convonly model")
        cnn_model = simplePatchCNN_convonly
    ...
    elif config["patch_net"]["model_type"] == "default":
        modeltype = "default"
        print("using default model")
        cnn_model = simplePatchCNN
